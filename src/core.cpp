...
List find_strongest_pairs(List pairs, NumericMatrix X, NumericVector Y, NumericVector weights, int max_number_of_pairs, bool standardize) {
  ...
  for(int m = 0; m < size_of_list; ++m) {
    ...
    temps = absolute_covariates_pairs(tempp,X,Y,weights,standardize);
  ...
  temp_strength = absolute_covariates_pairs(temp_pairs,X,Y,weights,standardize);
...
//[[Rcpp::export]]
List naive_interaction_search(NumericMatrix X, NumericVector Y, NumericVector weights,
                              int max_number_of_pairs, bool standardize) {
  int n = X.nrow();
  int p = X.ncol();
...
List interaction_search(NumericMatrix X, NumericVector Y, NumericVector weights,
  ...
  List result(2);
  if(p < 20) {
    result = naive_interaction_search(X,Y,weights,max_number_of_pairs,standardize);
  } else {
...
NumericVector create_lambda_sequence(double max_cov, double alpha, int n_lambda, double eps = 0.001, double factor_eps_inv = 100.0) {
    ...
    if (std::abs(double (alpha)) < eps) alpha = eps;
...
bool scan_main_effects(const NumericMatrix &X, const NumericVector &Y, const NumericVector weights, List main_effects, List beta_main,
    ...
    NumericVector new_beta_main(temp_beta_main.size()+coefs.size());
...
bool scan_intr_effects(const NumericMatrix &X, const NumericVector &residuals, const IntegerMatrix &X_bin, const NumericVector &weights,
    ...
    result_interaction_search = interaction_search_low_level(X_bin,X,residuals,weights,projections,20,standardize);
...
NumericVector calculate_xbeta(const NumericMatrix &main_vars, const NumericMatrix &intr_vars, const NumericVector &Y,
    ...
    if (temp_main_effects.size() > 0) {
        for (int l = 0; l < temp_main_effects.size(); ++l) {
            for (int i = 0; i < n; ++i) {
                xbeta[i] = xbeta[i]+main_vars(i,temp_main_effects[l])*temp_beta_main[l];
            }
        }
    }
...
NumericVector calculate_residuals(const NumericMatrix &main_vars, const NumericMatrix &intr_vars, const NumericVector &Y,
    ...
    int n = Y.size();
    NumericVector res(n);
    res = Y-calculate_xbeta(main_vars,intr_vars,Y,weights,intercept,main_effects,beta_main,intr_effects,beta_intr,r,standardize);
    return res;
}
        ...
        if (nr_main > 0) {
            for (int l = 0; l < nr_main; ++l) {
                ...
                for (int i = 0; i < n; ++i) {
                    ...
                    temp = weights[i]*main_vars(i,variable);
                    sum1 += temp*residuals[i];
                    sum2 += temp*main_vars(i,variable);
                }
                ...
                //if (r==6 && l==6) Rcout <<beta_tilde <<" "<< delta <<" "<<count_iterations << "\n";
                if (std::abs(delta) > update_precision) {
                    converged = false;

                    /* update residuals */
                    for (int i = 0; i < n; ++i) {
                        residuals[i] = residuals[i]+main_vars(i,variable)*delta;
                    }
...
List gaussiglmnet(NumericMatrix X, NumericVector Y, NumericVector weights, NumericVector lambdas,
    ...
    NumericMatrix main_vars = scale_main(X,weights,standardize);
    intercept[0]=mean(Y);
    ...
    for (int r = 0; r < n_lambda; ++r) {
        ...
        for (int iter = 0; iter < max_outer; ++iter) {
            ...
            changed = true;
            changed = scan_main_effects(main_vars,residuals,weights,main_effects,beta_main,lambdas,alpha,standardize,r,add_max_main,true);
